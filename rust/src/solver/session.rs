...
use super::integrator::{step_bdf_vsvo, NordsieckHistory};
...
impl SolverHandle {
    #[new]
    pub fn new(lib_path: String, n: usize, bw: isize, y0: Vec<f64>, ydot0: Vec<f64>, id: Vec<f64>, p: Vec<f64>, spatial_diag: Vec<f64>, _debug: bool) -> PyResult<Self> {
        let lib = unsafe { libloading::Library::new(&lib_path).expect("Failed to load JIT library.") };
        let res_fn: NativeResFn = unsafe { *lib.get(b"evaluate_residual\0").unwrap() };
        let jac_fn: NativeJacFn = unsafe { *lib.get(b"evaluate_jacobian\0").unwrap() };
        let jvp_fn: Option<NativeJvpFn> = unsafe { lib.get(b"evaluate_jvp\0").map(|s| *s).ok() };
        
        let mut history = NordsieckHistory::new(n);
        history.z[0].copy_from_slice(&y0);
        history.z[1].copy_from_slice(&ydot0);
    ...
    /// Fast-forwards the manifold specifically to align the 0D DAE boundaries before resuming
    /// normal time progression. Critical for seamlessly hot-swapping cycler conditions.
    ...
    pub fn get_jacobian<'py>(&mut self, py: Python<'py>, c_j: f64) -> PyResult<Bound<'py, numpy::PyArray2<f64>>> {
        ...
        let jac_2d = numpy::ndarray::Array2::from_shape_vec((self.n, self.n), self.jac_buffer.clone()).unwrap();
...
impl SolverHandle {
    pub fn step_with_history(&mut self, dt: f64, hist: Option<&mut Vec<(f64, Vec<f64>, Vec<f64>)>>) -> PyResult<()> {
        step_bdf_vsvo(
            self.n, self.bw, &mut self.y, &mut self.ydot, &self.p, &self.id, &self.spatial_diag,
            dt, &mut self.current_dt, &mut self.history,
