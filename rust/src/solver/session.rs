...
impl SolverHandle {
    #[new]
    pub fn new(lib_path: String, n: usize, bw: isize, y0: Vec<f64>, ydot0: Vec<f64>, id: Vec<f64>, p: Vec<f64>, m: Vec<f64>, precond_diag: Vec<f64>) -> PyResult<Self> {
        ...
        let jvp_fn: Option<NativeJvpFn> = unsafe { lib.get::<NativeJvpFn>(b"evaluate_jvp\0").map(|sym| *sym).ok() };
        ...
        let mut handle = SolverHandle { 
            ...
            t: 0.0, y: y0, ydot: ydot0, id, p, m, precond_diag,
    ...
    pub fn step(&mut self, dt: f64) -> PyResult<()> {
        ...
        integrator.step(
            self.n, self.bw, &mut self.y, &mut self.ydot, &self.p, &self.m, &self.id, &self.precond_diag, dt, 
    ...
    pub fn reach_steady_state(&mut self) -> PyResult<()> {
        ...
        for _ in 0..100 {
            ...
            if self.bw == -1 {
                let jvp = self.jvp_fn.expect("evaluate_jvp not found.");
                let y_ptr = self.y.as_ptr();
                let ydot_ptr = self.ydot.as_ptr();
                let p_ptr = self.p.as_ptr();
                let m_ptr = self.m.as_ptr();
                
                let jvp_closure = |v: &[f64], out: &mut [f64]| {
                    unsafe { jvp(y_ptr, ydot_ptr, p_ptr, m_ptr, 0.0, v.as_ptr(), out.as_mut_ptr()) };
                };
                let precond = |v: &[f64], out: &mut [f64]| {
                    for i in 0..self.n { out[i] = v[i] / (0.0 * self.id[i] + self.precond_diag[i]); }
                };
    ...
    pub fn get_jacobian<'py>(&self, py: Python<'py>, c_j: f64) -> Bound<'py, PyArray2<f64>> {
        let mut jac = vec![0.0; self.n * self.n];
        unsafe { (self.jac_fn)(self.y.as_ptr(), self.ydot.as_ptr(), self.p.as_ptr(), self.m.as_ptr(), c_j, jac.as_mut_ptr()) };
    ...
    fn initialize_ic(&mut self) -> PyResult<()> {
        ...
        for _ in 0..20 {
            ...
            if self.bw == -1 {
                ...
                let jvp_closure = |v: &[f64], out: &mut [f64]| {
                    unsafe { 
                        jvp(y_ptr, ydot_ptr, p_ptr, m_ptr, 0.0, v.as_ptr(), out.as_mut_ptr()); 
